...
app = Flask(__name__)
app.secret_key = os.getenv("FLASK_SECRET_KEY", "gizli-anahtar-varsayilan")

# CORS yapılandırmasını düzelt: origins="*" iken supports_credentials=True çalışmaz.
# Frontend adresinizi (genelde http://localhost:3000) buraya ekleyin.
...
def init_db():
    ...
    # Örnek kullanıcıları sadece tablo boşsa ekle
    ...
    conn.commit()
    conn.close()
    print("Veritabanı kontrol edildi/güncellendi.")
...
def get_vlm_analysis(image_path):
    try:
        ...
        print("\nSending VLM request to Gemini API...")
